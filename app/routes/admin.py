...
def get_task_status():
    ...
    try:
        ...
        for course in training_courses:
            # 당일 체크된 데이터만 필터링
            daily_checklist = session.query(TaskChecklist).filter(
...
def get_overall_task_status():
    ...
    try:
        ...
        for course in training_courses:
            # 전체 체크리스트 데이터
            all_checklist = session.query(TaskChecklist).filter(
                TaskChecklist.training_course == course.training_course
            ).all()
...
def get_combined_task_status():
    ...
    try:
        ...
        training_courses = session.query(TrainingInfo).filter(
            TrainingInfo.end_date >= one_week_ago
        ).order_by(TrainingInfo.end_date.desc()).all()
        ...
        for course in training_courses:
            ...
            # 당일 체크리스트 데이터
            daily_checklist = [
                task for task in all_checklist 
